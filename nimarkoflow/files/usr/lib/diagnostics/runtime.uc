...
const CONFIG_NAME = getenv("NIMARKOFLOW_CONFIG_NAME") || constants.NIMARKOFLOW_CONFIG_NAME || "nimarkoflow";
...
const NIMARKOFLOW_CONFIG = getenv("NIMARKOFLOW_CONFIG") || constants.NIMARKOFLOW_CONFIG || "/etc/config/" + CONFIG_NAME;
const NIMARKOFLOW_SERVICE_NAME = getenv("NIMARKOFLOW_SERVICE_NAME") || constants.NIMARKOFLOW_SERVICE_NAME || "nimarkoflow";
const NIMARKOFLOW_RELEASE_REPO = getenv("NIMARKOFLOW_RELEASE_REPO") || constants.NIMARKOFLOW_RELEASE_REPO || "";
const NIMARKOFLOW_LUCI_VIEW_DIR = getenv("NIMARKOFLOW_LUCI_VIEW_DIR") || constants.NIMARKOFLOW_LUCI_VIEW_DIR || "/www/luci-static/resources/view/nimarkoflow";
...
const SYSTEM_INFO_CACHE_TTL = int(getenv("NIMARKOFLOW_SYSTEM_INFO_CACHE_TTL") || "3600");
...
const RT_TABLE_NAME = getenv("RT_TABLE_NAME") || constants.RT_TABLE_NAME || "nimarkoflow";
const NFT_TABLE_NAME = getenv("NFT_TABLE_NAME") || constants.NFT_TABLE_NAME || "NimarkoFlowTable";
const NFT_FAKEIP_MARK = getenv("NFT_FAKEIP_MARK") || constants.NFT_FAKEIP_MARK || "0x04000000";
const NFT_COMMON_SET_NAME = getenv("NFT_COMMON_SET_NAME") || constants.NFT_COMMON_SET_NAME || "nimarkoflow_subnets";
const NFT_PORT_SET_NAME = getenv("NFT_PORT_SET_NAME") || constants.NFT_PORT_SET_NAME || "nimarkoflow_ports";
const NFT_IP_PORT_SET_NAME = getenv("NFT_IP_PORT_SET_NAME") || constants.NFT_IP_PORT_SET_NAME || "nimarkoflow_ip_ports";
const NFT_INTERFACE_SET_NAME = getenv("NFT_INTERFACE_SET_NAME") || constants.NFT_INTERFACE_SET_NAME || "nimarkoflow_interfaces";
const NFT_DISCORD_SET_NAME = getenv("NFT_DISCORD_SET_NAME") || constants.NFT_DISCORD_SET_NAME || "nimarkoflow_discord_subnets";
...
const SB_VARIANT_STATE_FILE = getenv("SB_VARIANT_STATE_FILE") || constants.SB_VARIANT_STATE_FILE || "/etc/nimarkoflow/sing-box-variant";
const SING_BOX_BIN_PATH = getenv("NIMARKOFLOW_DIAGNOSTICS_SING_BOX_BIN_PATH") || "/usr/bin/sing-box";
...
function check_logs() {
    ...
    let rendered = status_capture([ "nimarkoflow-logs" ], command_output_from_args([ "logread" ]));
    if (rendered.output != "")
...
function nimarkoflow_logs_fixture() {
    let rendered = status_capture([ "nimarkoflow-logs" ], read_stdin());
    if (rendered.output != "")
        print(rendered.output);
...
function show_config(visibility) {
    visibility = as_string(visibility || "masked");
    if (!file_exists(NIMARKOFLOW_CONFIG)) {
        nolog("Configuration file not found");
        return 1;
    }
    if (visibility == "raw")
        print(as_string(fs.readfile(NIMARKOFLOW_CONFIG)));
    else
        print(status_output([ "nimarkoflow-config-masked", NIMARKOFLOW_CONFIG ], null));
    return 0;
}

function show_version() {
    print(NIMARKOFLOW_VERSION, "\n");
    return 0;
}
...
function get_luci_app_version() {
    let path = NIMARKOFLOW_LUCI_VIEW_DIR + "/main.js";
    ...
    for (let line in split(as_string(data), "\n")) {
        ...
        if (matched != null && matched[1] == "NIMARKOFLOW_LUCI_APP_VERSION")
...
function system_info_cache_is_valid() {
    ...
    if (now > 0 && generated_at > 0 && SYSTEM_INFO_CACHE_TTL > 0 && now - generated_at >= SYSTEM_INFO_CACHE_TTL)
        return false;
    return cache.nimarkoflow_version == NIMARKOFLOW_VERSION && cache.luci_app_version == get_luci_app_version();
}
...
function build_system_info() {
    let nimarkoflow_latest_version = first_line_value("/tmp/nimarkoflow.latest-version.cache", "unknown");
    ...
    return {
        nimarkoflow_version: NIMARKOFLOW_VERSION,
        nimarkoflow_latest_version: nimarkoflow_latest_version || "unknown",
        luci_app_version,
        sing_box_version,
...
function neutralize_zapret_defaults() {
    log_message("Standalone zapret is not neutralized automatically; NimarkoFlow uses /opt/zapret/nfq/nfqws as an external provider and manages only its own NFQUEUE range.", "info");
    return 0;
}
...
function dnsmasq_has_nimarkoflow_dns() {
    return module_success(DNS_APPLY_UC, [ "has-nimarkoflow-dns" ]);
}
...
function get_sing_box_status() {
    ...
    let dns_configured = dnsmasq_has_nimarkoflow_dns() ? 1 : 0;
...
function get_status() {
    let running = module_success(SERVICE_STATE_UC, [
        "nimarkoflow-stably-running", RT_TABLE_NAME, NFT_TABLE_NAME, NFT_FAKEIP_MARK, RUNTIME_STABLE_MIN_AGE
    ]) ? 1 : 0;
    let enabled = file_executable("/etc/rc.d/S99" + NIMARKOFLOW_SERVICE_NAME) ? 1 : 0;
    let dns_configured = dnsmasq_has_nimarkoflow_dns() ? 1 : 0;
    write_service_status(running, enabled, dns_configured);
    return 0;
}
...
function global_check(arg1, arg2) {
    ...
    print_global("━━━━━━━━━━━━━━━━━━━━━━━━━━━");
    print_global("📄 NimarkoFlow config");
    ...
    if (file_executable("/etc/init.d/zapret") && command_success_from_args([ "/etc/init.d/zapret", "status" ])) {
        print_global("━━━━━━━━━━━━━━━━━━━━━━━━━━━");
        print_global("⚠️ Standalone zapret service is active. NimarkoFlow uses separate queues, but packet-level policy overlap is possible.");
    }
    else if (file_executable("/etc/init.d/zapret") && command_success_from_args([ "/etc/init.d/zapret", "enabled" ])) {
        print_global("━━━━━━━━━━━━━━━━━━━━━━━━━━━");
        print_global("⚠️ Standalone zapret autostart is enabled. NimarkoFlow will not modify /etc/config/zapret.");
    }

    if (file_executable("/etc/init.d/zapret2") && command_success_from_args([ "/etc/init.d/zapret2", "status" ])) {
        print_global("━━━━━━━━━━━━━━━━━━━━━━━━━━━");
        print_global("⚠️ Standalone zapret2 service is active. NimarkoFlow uses separate queues, but packet-level policy overlap is possible.");
    }
    else if (file_executable("/etc/init.d/zapret2") && command_success_from_args([ "/etc/init.d/zapret2", "enabled" ])) {
        print_global("━━━━━━━━━━━━━━━━━━━━━━━━━━━");
        print_global("⚠️ Standalone zapret2 autostart is enabled. NimarkoFlow will not modify /etc/config/zapret2.");
    }
...
else if (mode == "nimarkoflow-logs-fixture")
    exit(nimarkoflow_logs_fixture());
