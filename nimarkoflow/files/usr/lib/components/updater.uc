...
function nimarkoflow_release_plan(latest_version, asset_ext, i18n_required) {
    let release = object_or_empty(read_stdin_json());
    ...
    let backend = release_asset_pair(release, "nimarkoflow_" + latest_version + "." + asset_ext);
    let app = release_asset_pair(release, "luci-app-nimarkoflow_" + latest_version + "." + asset_ext);
    if (backend == null || app == null)
        exit(1);

    let i18n = { name: "", url: "" };
    if (arg_bool(i18n_required)) {
        i18n = release_asset_pair(release, "luci-i18n-nimarkoflow-ru_" + latest_version + "." + asset_ext);
        if (i18n == null)
            exit(1);
    }
...
function nimarkoflow_normalized_release_version(value) {
    return as_string(value);
}

function nimarkoflow_release_version_valid(value) {
    value = nimarkoflow_normalized_release_version(value);
    return match(value, /^[0-9]+[.][0-9]+[.][0-9]+$/) != null;
}

function nimarkoflow_release_version_parts(value) {
    let version = nimarkoflow_normalized_release_version(value);
    if (!nimarkoflow_release_version_valid(version))
        return null;
...
function nimarkoflow_release_version_compare(lhs, rhs) {
    let lhs_parts = nimarkoflow_release_version_parts(lhs);
    let rhs_parts = nimarkoflow_release_version_parts(rhs);
...
else if (mode == "nimarkoflow-release-plan")
    nimarkoflow_release_plan(ARGV[1], ARGV[2], ARGV[3]);
...
else if (mode == "nimarkoflow-release-version-valid")
    exit(nimarkoflow_release_version_valid(ARGV[1]) ? 0 : 1);
else if (mode == "nimarkoflow-release-version-compare")
    exit(nimarkoflow_release_version_compare(ARGV[1], ARGV[2]) ? 0 : 1);
