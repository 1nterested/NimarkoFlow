...
const NIMARKOFLOW_RUNTIME_STATE_DIR = getenv("NIMARKOFLOW_RUNTIME_STATE_DIR") || "/var/run/nimarkoflow";
const NIMARKOFLOW_SUBSCRIPTION_UPDATE_STATE_DIR = getenv("NIMARKOFLOW_SUBSCRIPTION_UPDATE_STATE_DIR") || NIMARKOFLOW_RUNTIME_STATE_DIR + "/subscription-update";
const NIMARKOFLOW_SUBSCRIPTION_LINKS_DIR = getenv("NIMARKOFLOW_SUBSCRIPTION_LINKS_DIR") || NIMARKOFLOW_RUNTIME_STATE_DIR + "/subscription-links";
const NIMARKOFLOW_SUBSCRIPTION_METADATA_DIR = getenv("NIMARKOFLOW_SUBSCRIPTION_METADATA_DIR") || NIMARKOFLOW_RUNTIME_STATE_DIR + "/subscription-metadata";
const NIMARKOFLOW_OUTBOUND_METADATA_DIR = getenv("NIMARKOFLOW_OUTBOUND_METADATA_DIR") || NIMARKOFLOW_RUNTIME_STATE_DIR + "/outbound-metadata";
const NIMARKOFLOW_SECTION_CACHE_DIR = getenv("NIMARKOFLOW_SECTION_CACHE_DIR") || NIMARKOFLOW_RUNTIME_STATE_DIR + "/section-cache";
const NIMARKOFLOW_RUNTIME_CACHE_FORMAT_FILE = getenv("NIMARKOFLOW_RUNTIME_CACHE_FORMAT_FILE") || NIMARKOFLOW_RUNTIME_STATE_DIR + "/cache-format";
const NIMARKOFLOW_RUNTIME_CACHE_FORMAT = getenv("NIMARKOFLOW_RUNTIME_CACHE_FORMAT") || "8";
const NIMARKOFLOW_PERSISTENT_SUBSCRIPTION_CACHE_DIR = getenv("NIMARKOFLOW_PERSISTENT_SUBSCRIPTION_CACHE_DIR") || "/etc/nimarkoflow/subscription-cache";
const NIMARKOFLOW_PERSISTENT_SUBSCRIPTION_CACHE_FORMAT_FILE = getenv("NIMARKOFLOW_PERSISTENT_SUBSCRIPTION_CACHE_FORMAT_FILE") || NIMARKOFLOW_PERSISTENT_SUBSCRIPTION_CACHE_DIR + "/cache-format";
const NIMARKOFLOW_PERSISTENT_SUBSCRIPTION_CACHE_FORMAT = getenv("NIMARKOFLOW_PERSISTENT_SUBSCRIPTION_CACHE_FORMAT") || "7";
const NIMARKOFLOW_SUBSCRIPTION_BOOTSTRAP_RETRY_PID_FILE = getenv("NIMARKOFLOW_SUBSCRIPTION_BOOTSTRAP_RETRY_PID_FILE") || NIMARKOFLOW_RUNTIME_STATE_DIR + "/subscription-bootstrap-retry.pid";
const NIMARKOFLOW_SUBSCRIPTION_UPDATE_LOCK_DIR = getenv("NIMARKOFLOW_SUBSCRIPTION_UPDATE_LOCK_DIR") || NIMARKOFLOW_RUNTIME_STATE_DIR + "/subscription-update.lock";
const NIMARKOFLOW_PENDING_RELOAD_FILE = getenv("NIMARKOFLOW_PENDING_RELOAD_FILE") || NIMARKOFLOW_RUNTIME_STATE_DIR + "/reload.pending";
const NIMARKOFLOW_SERVICE_INIT = getenv("NIMARKOFLOW_SERVICE_INIT") || "/etc/init.d/nimarkoflow";
...
const SB_VARIANT_STATE_FILE = getenv("SB_VARIANT_STATE_FILE") || "/etc/nimarkoflow/sing-box-variant";
const SB_VERSION_STATE_FILE = getenv("SB_VERSION_STATE_FILE") || "/etc/nimarkoflow/sing-box-version";
...
function subscription_metadata_path(section) {
    ...
    if (!cache_section_is_safe(section))
        return "";
    return NIMARKOFLOW_SUBSCRIPTION_METADATA_DIR + "/" + section + ".json";
}
...
function outbound_metadata_path(section) {
    ...
    if (!cache_section_is_safe(section))
        return "";
    return NIMARKOFLOW_OUTBOUND_METADATA_DIR + "/" + section + ".json";
}
...
function ensure_runtime_dirs() {
    ...
    ensure_dir(NIMARKOFLOW_RUNTIME_STATE_DIR);
    ensure_dir(NIMARKOFLOW_SUBSCRIPTION_UPDATE_STATE_DIR);
    ensure_dir(NIMARKOFLOW_SUBSCRIPTION_METADATA_DIR);
    ensure_dir(NIMARKOFLOW_OUTBOUND_METADATA_DIR);
    ensure_dir(NIMARKOFLOW_SECTION_CACHE_DIR);
}

function clear_subscription_runtime_cache() {
    run_silent("rm -rf " +
        shell_quote(TMP_SUBSCRIPTION_FOLDER) + " " +
        shell_quote(NIMARKOFLOW_SUBSCRIPTION_LINKS_DIR) + " " +
        shell_quote(NIMARKOFLOW_SUBSCRIPTION_METADATA_DIR) + " " +
        shell_quote(NIMARKOFLOW_OUTBOUND_METADATA_DIR) + " " +
        shell_quote(NIMARKOFLOW_SECTION_CACHE_DIR));
}

function ensure_runtime_cache_format() {
    ensure_dir(NIMARKOFLOW_RUNTIME_STATE_DIR);

    if (file_first_line_value(NIMARKOFLOW_RUNTIME_CACHE_FORMAT_FILE) != NIMARKOFLOW_RUNTIME_CACHE_FORMAT) {
        log_message("Runtime subscription cache format changed; clearing old subscription cache", "info");
        if (file_first_line_value(NIMARKOFLOW_PERSISTENT_SUBSCRIPTION_CACHE_FORMAT_FILE) == NIMARKOFLOW_PERSISTENT_SUBSCRIPTION_CACHE_FORMAT)
            subscription_share_link.populate_subscription_dir(NIMARKOFLOW_PERSISTENT_SUBSCRIPTION_CACHE_DIR);
        clear_subscription_runtime_cache();
        ensure_runtime_dirs();
        write_file(NIMARKOFLOW_RUNTIME_CACHE_FORMAT_FILE, NIMARKOFLOW_RUNTIME_CACHE_FORMAT + "\n");
    }

    if (file_first_line_value(NIMARKOFLOW_PERSISTENT_SUBSCRIPTION_CACHE_FORMAT_FILE) != NIMARKOFLOW_PERSISTENT_SUBSCRIPTION_CACHE_FORMAT) {
        run_silent("rm -rf " + shell_quote(NIMARKOFLOW_PERSISTENT_SUBSCRIPTION_CACHE_DIR));
        ensure_dir(NIMARKOFLOW_PERSISTENT_SUBSCRIPTION_CACHE_DIR);
        chmod_path(NIMARKOFLOW_PERSISTENT_SUBSCRIPTION_CACHE_DIR, "700");
        write_file(NIMARKOFLOW_PERSISTENT_SUBSCRIPTION_CACHE_FORMAT_FILE, NIMARKOFLOW_PERSISTENT_SUBSCRIPTION_CACHE_FORMAT + "\n");
        chmod_path(NIMARKOFLOW_PERSISTENT_SUBSCRIPTION_CACHE_FORMAT_FILE, "600");
    }
}
...
function section_current_usable_cache_by_name(sections, section, default_user_agent) {
    return section_current_usable_cache(find_section(sections, section), TMP_SUBSCRIPTION_FOLDER, NIMARKOFLOW_PERSISTENT_SUBSCRIPTION_CACHE_DIR, default_user_agent);
}
...
function cache_candidate_paths() {
    ...
    for (let dir in [
        ...
        NIMARKOFLOW_PERSISTENT_SUBSCRIPTION_CACHE_DIR,
        NIMARKOFLOW_SECTION_CACHE_DIR,
        NIMARKOFLOW_SUBSCRIPTION_LINKS_DIR,
        NIMARKOFLOW_SUBSCRIPTION_METADATA_DIR,
        NIMARKOFLOW_OUTBOUND_METADATA_DIR
    ]) {
...
function persistent_metadata_path(source_section) {
    return NIMARKOFLOW_PERSISTENT_SUBSCRIPTION_CACHE_DIR + "/" + as_string(source_section) + ".metadata.json";
}
...
function persist_subscription_cache(source_section, subscription_json_path, subscription_url, effective_user_agent, effective_hwid, metadata_path) {
    ...
    let persistent_json = source_json_path(NIMARKOFLOW_PERSISTENT_SUBSCRIPTION_CACHE_DIR, source_section);
    let persistent_url = source_url_path(NIMARKOFLOW_PERSISTENT_SUBSCRIPTION_CACHE_DIR, source_section);
    let persistent_user_agent = source_user_agent_path(NIMARKOFLOW_PERSISTENT_SUBSCRIPTION_CACHE_DIR, source_section);
    let persistent_hwid = source_hwid_path(NIMARKOFLOW_PERSISTENT_SUBSCRIPTION_CACHE_DIR, source_section);
...
function append_available_cached_metadata(array_path, section, source_index, source_section) {
    ...
    append_persistent_source_metadata(array_path, source_index, source_section) ||
        append_cached_metadata(array_path, NIMARKOFLOW_SECTION_CACHE_DIR, section, NIMARKOFLOW_SUBSCRIPTION_METADATA_DIR + "/" + section + ".json", source_index, source_section);
...
function get_subscription_user_agent(custom_user_agent) {
    ...
    return "Happ/5.7.0/ios/2609021014549 NimarkoFlow/0.1.0";
}
...
function subscription_update_timestamp_path(section_name_value) {
    return NIMARKOFLOW_SUBSCRIPTION_UPDATE_STATE_DIR + "/" + as_string(section_name_value) + ".timestamp";
}
...
function finalize_subscription_section_metadata(section_name_value, metadata_tmpfile, superseded) {
    ...
    if (!superseded) {
        if (length(array_or_empty(read_json(metadata_tmpfile))) > 0)
            write_subscription_metadata(NIMARKOFLOW_SECTION_CACHE_DIR, NIMARKOFLOW_RUNTIME_CACHE_FORMAT, section_name_value, metadata_tmpfile);
        else
            write_subscription_metadata(NIMARKOFLOW_SECTION_CACHE_DIR, NIMARKOFLOW_RUNTIME_CACHE_FORMAT, section_name_value, "");
    }
...
function subscription_update_selected_source(sections, section_name_value, source_index_value, force) {
    ...
    if (update_result == 0 || update_result == 2)
        write_source_metadata(
            NIMARKOFLOW_SECTION_CACHE_DIR,
            NIMARKOFLOW_RUNTIME_CACHE_FORMAT,
            section_name_value,
            source_index,
            source_section,
            metadata_output_path,
            NIMARKOFLOW_SUBSCRIPTION_METADATA_DIR + "/" + section_name_value + ".json"
        );
...
function prepare_subscription_caches(phase, already_prepared, no_refresh) {
    ...
    if (prepared_runtime_cache_should_skip(sections, NIMARKOFLOW_SECTION_CACHE_DIR, phase, already_prepared)) {
        print("\n");
        return 0;
    }
...
function mark_pending_subscription_recovery_reload() {
    state_ucode_status([ "mark-pending-reload", NIMARKOFLOW_PENDING_RELOAD_FILE, "subscription_deferred_recovery" ]);
}

function trigger_subscription_recovery_reload(worker) {
    if (worker)
        command_status_from_args([ NIMARKOFLOW_SERVICE_INIT, "reload", "subscription_deferred_recovery" ]);
    else
...
function worker_env() {
    return {
        ...
        TMP_RULESET_FOLDER,
        TMP_SUBSCRIPTION_FOLDER,
        NIMARKOFLOW_RUNTIME_STATE_DIR,
        NIMARKOFLOW_SUBSCRIPTION_UPDATE_STATE_DIR,
        NIMARKOFLOW_SUBSCRIPTION_LINKS_DIR,
        NIMARKOFLOW_SUBSCRIPTION_METADATA_DIR,
        NIMARKOFLOW_OUTBOUND_METADATA_DIR,
        NIMARKOFLOW_SECTION_CACHE_DIR,
        NIMARKOFLOW_RUNTIME_CACHE_FORMAT_FILE,
        NIMARKOFLOW_RUNTIME_CACHE_FORMAT,
        NIMARKOFLOW_PERSISTENT_SUBSCRIPTION_CACHE_DIR,
        NIMARKOFLOW_PERSISTENT_SUBSCRIPTION_CACHE_FORMAT_FILE,
        NIMARKOFLOW_PERSISTENT_SUBSCRIPTION_CACHE_FORMAT,
        NIMARKOFLOW_SUBSCRIPTION_BOOTSTRAP_RETRY_PID_FILE,
        NIMARKOFLOW_SUBSCRIPTION_UPDATE_LOCK_DIR,
        NIMARKOFLOW_PENDING_RELOAD_FILE,
        NIMARKOFLOW_SERVICE_INIT,
...
function start_deferred_subscription_bootstrap_retry_worker(deferred_sections) {
    ...
    ensure_runtime_dirs();
    let existing_pid = trim(file_first_line_value(NIMARKOFLOW_SUBSCRIPTION_BOOTSTRAP_RETRY_PID_FILE));
    ...
    if (pid != "")
        write_file(NIMARKOFLOW_SUBSCRIPTION_BOOTSTRAP_RETRY_PID_FILE, pid + "\n");
...
function stop_deferred_subscription_bootstrap_retry_worker() {
    let pid = trim(file_first_line_value(NIMARKOFLOW_SUBSCRIPTION_BOOTSTRAP_RETRY_PID_FILE));
    if (pid_running(pid)) {
        command_success_from_args([ "kill", pid ]);
        log_message("Stopped subscription bootstrap retry worker", "info");
    }
    unlink_path(NIMARKOFLOW_SUBSCRIPTION_BOOTSTRAP_RETRY_PID_FILE);
}
...
function run_deferred_subscription_bootstrap(deferred_sections) {
    ...
    if (result.recovered != "") {
        log_message("Recovered deferred subscription rule(s): " + result.recovered + "; scheduling NimarkoFlow reload", "info");
...
function deferred_subscription_bootstrap_retry_worker(remaining_sections) {
    ...
    while (remaining_sections != "") {
        ...
        ensure_runtime_dirs();
        if (!state_ucode_status([ "acquire-runtime-dir-lock", NIMARKOFLOW_SUBSCRIPTION_UPDATE_LOCK_DIR, current_pid() ])) {
        ...
        state_ucode_status([ "release-runtime-dir-lock", NIMARKOFLOW_SUBSCRIPTION_UPDATE_LOCK_DIR ]);

        if (result.recovered != "") {
            log_message("Recovered deferred subscription rule(s): " + result.recovered + "; reloading NimarkoFlow", "info");
