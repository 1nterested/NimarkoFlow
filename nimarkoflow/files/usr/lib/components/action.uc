...
const BIN_PATH = getenv("NIMARKOFLOW_BIN") || constants.NIMARKOFLOW_BIN || "/usr/bin/nimarkoflow";
const SERVICE_INIT = getenv("NIMARKOFLOW_SERVICE_INIT") || constants.NIMARKOFLOW_SERVICE_INIT || "/etc/init.d/nimarkoflow";
const NIMARKOFLOW_VERSION = getenv("NIMARKOFLOW_VERSION") || constants.NIMARKOFLOW_VERSION || "";
const NIMARKOFLOW_RELEASE_REPO = getenv("NIMARKOFLOW_RELEASE_REPO") || constants.NIMARKOFLOW_RELEASE_REPO || "";
...
const SB_MANAGED_SERVICE_MARKER = getenv("SB_MANAGED_SERVICE_MARKER") || constants.SB_MANAGED_SERVICE_MARKER || "NimarkoFlow managed sing-box service for binary variants";

let tmp_dir = "";
let lock_held = false;
let nimarkoflow_was_running = false;
let nimarkoflow_stopped_for_sing_box_change = false;
...
function log_message(message, level) {
    ...
    command_success_from_args([ "logger", "-t", "nimarkoflow", "[" + level + "] " + as_string(message) ]);
}
...
function cleanup_stale_tmp_files() {
    command_success_from_args([ "find", "/tmp", "-maxdepth", "1", "-type", "d", "-name", "nimarkoflow-updates.*", "-mmin", "+" + as_string(TMP_STALE_TTL_MINUTES), "-exec", "rm", "-rf", "{}", "+" ]);
    command_success_from_args([ "find", "/tmp", "-maxdepth", "1", "-type", "f", "(", "-name", "nimarkoflow-updates-command.*", "-o", "-name", "nimarkoflow-updates-http.*", ")", "-mmin", "+" + as_string(TMP_FILE_STALE_TTL_MINUTES), "-delete" ]);
}
...
function init_tmp_dir() {
    ...
    tmp_dir = trim(command_output_from_args([ "mktemp", "-d", "/tmp/nimarkoflow-updates.XXXXXX" ]));
    if (tmp_dir == "") {
        tmp_dir = "/tmp/nimarkoflow-updates." + owner_pid();
...
function make_tmp_file(prefix) {
    init_tmp_dir();
    let base = tmp_dir != "" ? tmp_dir + "/" + as_string(prefix) + ".XXXXXX" : "/tmp/nimarkoflow-updates-" + as_string(prefix) + ".XXXXXX";
...
function restart_nimarkoflow_after_failed_sing_box_change() {
    if (!nimarkoflow_stopped_for_sing_box_change || !nimarkoflow_was_running || !file_exists(SERVICE_INIT))
        return;
    updates_log("Restarting NimarkoFlow after failed sing-box component change");
...
function action_fail(component, action, message, current_version, latest_version, status, release_url) {
    ...
    restart_nimarkoflow_after_failed_sing_box_change();
...
function run_logged(description, command) {
    ...
    if (output_file == "")
        output_file = "/tmp/nimarkoflow-updates-command." + owner_pid();
...
function latest_nimarkoflow_release_json() {
    let parts = split(NIMARKOFLOW_RELEASE_REPO, "/");
    if (length(parts) != 2 || as_string(parts[0]) == "" || as_string(parts[1]) == "")
        return "";
    return fetch_github_release_json(parts[0], parts[1]);
}

function latest_nimarkoflow_version() {
    let response = latest_nimarkoflow_release_json();
    if (response == "")
        return "";
    return trim(helper_output_input(response, "object-get-default", [ "tag_name", "" ]));
}

function fetch_nimarkoflow_latest_release_metadata() {
    let response = latest_nimarkoflow_release_json();
    if (response == "")
        return "";
...
function write_nimarkoflow_latest_version_cache(value, timestamp) {
    if (as_string(value) == "")
        return;
    write_file("/tmp/nimarkoflow.latest-version.cache", as_string(value) + "\n" + as_string(timestamp) + "\n");
}
...
function clear_version_caches() {
    remove_file("/tmp/nimarkoflow.latest-version.cache");
    remove_file(SYSTEM_INFO_CACHE_FILE);
    remove_file("/tmp/nimarkoflow/system-info.json");
}
...
function install_managed_sing_box_service_script() {
    let tmp = "/etc/init.d/sing-box.nimarkoflow." + owner_pid();
...
function nimarkoflow_status_running_with_timeout() {
    init_tmp_dir();
    let output_file = make_tmp_file("nimarkoflow-status");
    if (output_file == "")
        return false;
...
function capture_nimarkoflow_running_state() {
    nimarkoflow_was_running = file_exists(BIN_PATH) && nimarkoflow_status_running_with_timeout();
}

function restart_nimarkoflow_after_successful_change() {
    if (!file_exists(SERVICE_INIT))
        return;
    if (!nimarkoflow_was_running) {
        updates_log("NimarkoFlow was not running before component change; restart skipped");
        prepare_sing_box_service_disabled();
        return;
    }
    run_logged("Restarting NimarkoFlow after successful component change", command_from_args([ SERVICE_INIT, "restart" ]));
}

function stop_nimarkoflow_before_sing_box_change() {
    if (nimarkoflow_stopped_for_sing_box_change)
        return;
    nimarkoflow_stopped_for_sing_box_change = true;

    if (nimarkoflow_was_running && file_exists(SERVICE_INIT))
        run_logged("Stopping NimarkoFlow before sing-box package change", command_from_args([ SERVICE_INIT, "stop" ]));

    if (nimarkoflow_was_running && file_exists(BIN_PATH))
        command_success_from_args([ BIN_PATH, "restore_dnsmasq" ]);

    prepare_sing_box_service_disabled();
}

function wait_nimarkoflow_running_after_sing_box_change() {
    if (!nimarkoflow_was_running)
        return true;
    ...
    let waited = 0;
    while (waited < 60) {
        if (nimarkoflow_status_running_with_timeout()) {
...
function install_zapret_like(component, action, runtime_module, resolve_fn, label) {
    ...
    restart_nimarkoflow_after_successful_change();
...
function move_file_portable(source_path, target_path) {
    ...
    let staged_path = as_string(target_path) + ".nimarkoflow-move." + owner_pid();
...
function install_staged_file(source_path, target_path, mode) {
    let staged_path = as_string(target_path) + ".nimarkoflow-new." + owner_pid();
    remove_file(staged_path);
...
function install_sing_box_extended_package(action) {
    ...
    stop_nimarkoflow_before_sing_box_change();
    prepare_sing_box_package_service_install();
    ...
    if (current_variant == "extended" || current_variant == "extended-compressed") {
        if (file_exists("/usr/bin/sing-box")) {
            backup_binary = "/usr/bin/sing-box.nimarkoflow-backup." + owner_pid();
        ...
        if (file_exists("/usr/lib/libcronet.so")) {
            cronet_touched = true;
            backup_cronet = "/usr/lib/libcronet.so.nimarkoflow-backup." + owner_pid();
    ...
    write_sing_box_variant_state("extended", new_version);
    restart_nimarkoflow_after_successful_change();
    if (!wait_nimarkoflow_running_after_sing_box_change()) {
        ...
        if (restore_sing_box_after_failed_extended_package_install(current_variant, backup_binary, backup_cronet, previous_marker, previous_version_state, package_file, cronet_touched)) {
            remove_file(backup_binary);
            remove_file(backup_cronet);
            action_fail("sing_box", action, "sing-box-extended package was installed but NimarkoFlow did not start cleanly; previous sing-box variant was restored", current_version, latest_version);
        }
        action_fail("sing_box", action, "sing-box-extended package was installed but NimarkoFlow did not start cleanly and previous sing-box variant could not be restored", current_version, latest_version);
    }
...
function install_sing_box_extended(action, compressed) {
    ...
    if (!wait_nimarkoflow_running_after_sing_box_change()) {
        ...
        if (restore_sing_box_after_failed_extended_install(current_variant, backup_binary, backup_cronet, previous_marker, previous_version_state, archive_file, cronet_touched)) {
            remove_file(backup_binary);
            remove_file(backup_cronet);
            action_fail("sing_box", action, label + " was installed but NimarkoFlow did not start cleanly; previous sing-box variant was restored", current_version, latest_version);
        }
        action_fail("sing_box", action, label + " was installed but NimarkoFlow did not start cleanly and previous sing-box variant could not be restored", current_version, latest_version);
    }
...
function install_package_sing_box(action, tiny) {
    ...
    if (file_exists("/usr/bin/sing-box")) {
        backup_binary = backup_on_tmpfs ? tmp_dir + "/sing-box.nimarkoflow-backup" :
            "/usr/bin/sing-box.nimarkoflow-backup." + owner_pid();
        if (!move_file_to_backup("/usr/bin/sing-box", backup_binary))
            action_fail("sing_box", action, "Failed to backup current sing-box binary", current_version, latest_version);
    }
    if (file_exists("/usr/lib/libcronet.so")) {
        cronet_touched = true;
        backup_cronet = backup_on_tmpfs ? tmp_dir + "/libcronet.so.nimarkoflow-backup" :
            "/usr/lib/libcronet.so.nimarkoflow-backup." + owner_pid();
        if (!move_file_to_backup("/usr/lib/libcronet.so", backup_cronet)) {
    ...
    restart_nimarkoflow_after_successful_change();
    if (!wait_nimarkoflow_running_after_sing_box_change())
        fail_package_sing_box_install(action, tiny, "was installed, but NimarkoFlow did not start cleanly", new_version, latest_version,
            package_name, previous_variant, backup_binary, backup_cronet, previous_marker, previous_version_state, cronet_touched);
    remove_file(backup_binary);
    remove_file(backup_cronet);
    clear_version_caches();
...
function check_nimarkoflow() {
    let metadata = fetch_nimarkoflow_latest_release_metadata();
    ...
    if (latest_version == "unknown")
        action_fail("nimarkoflow", "check_update", "Failed to check NimarkoFlow updates", NIMARKOFLOW_VERSION, latest_version);

    write_nimarkoflow_latest_version_cache(latest_version, now_seconds());
    if (!helper_success("nimarkoflow-release-version-valid", [ NIMARKOFLOW_VERSION ])) {
        updates_log("NimarkoFlow current version is not a release version (" + NIMARKOFLOW_VERSION + ")");
        action_success("nimarkoflow", "check_update", "Installed version is newer than release", NIMARKOFLOW_VERSION, latest_version, 0, "dev", release_url);
    }

    let compare = trim(helper_output("nimarkoflow-release-version-compare", [ NIMARKOFLOW_VERSION, latest_version ]));
    if (compare == "")
        action_fail("nimarkoflow", "check_update", "Failed to compare NimarkoFlow versions", NIMARKOFLOW_VERSION, latest_version);
    let status = status_from_compare(int(compare));
    if (status == "")
        action_fail("nimarkoflow", "check_update", "Failed to compare NimarkoFlow versions", NIMARKOFLOW_VERSION, latest_version);
    if (status == "latest") {
        updates_log("NimarkoFlow is already up to date (" + NIMARKOFLOW_VERSION + ")");
        action_success("nimarkoflow", "check_update", "Latest version is installed", NIMARKOFLOW_VERSION, latest_version, 0, status, release_url);
    }
    if (status == "outdated") {
        updates_log("NimarkoFlow update found: " + NIMARKOFLOW_VERSION + " -> " + latest_version);
        action_success("nimarkoflow", "check_update", "Update is available", NIMARKOFLOW_VERSION, latest_version, 0, status, release_url);
    }
    updates_log("NimarkoFlow installed version is newer than upstream release: " + NIMARKOFLOW_VERSION + " -> " + latest_version);
    action_success("nimarkoflow", "check_update", "Installed version is newer than release", NIMARKOFLOW_VERSION, latest_version, 0, status, release_url);
}

function resolve_nimarkoflow_release(latest_version) {
    let release_json = latest_nimarkoflow_release_json();
    if (release_json == "")
        return null;
    let asset_ext = is_apk() ? "apk" : "ipk";
    let i18n_required = pkg_is_installed("luci-i18n-nimarkoflow-ru") ? "1" : "0";
    let plan = trim(helper_output_input(release_json, "nimarkoflow-release-plan", [ latest_version, asset_ext, i18n_required ]));
...
function install_nimarkoflow() {
    let latest_version = latest_nimarkoflow_version();
    if (latest_version == "")
        latest_version = "unknown";
    if (latest_version == "unknown")
        action_fail("nimarkoflow", "install", "Failed to resolve NimarkoFlow release", NIMARKOFLOW_VERSION, latest_version);

    write_nimarkoflow_latest_version_cache(latest_version, now_seconds());
    init_tmp_dir() || action_fail("nimarkoflow", "install", "Failed to create temporary directory", NIMARKOFLOW_VERSION, latest_version);
    updates_log("Resolving NimarkoFlow release " + latest_version + " packages");
    let release = resolve_nimarkoflow_release(latest_version);
    if (release == null)
        action_fail("nimarkoflow", "install", "Failed to resolve NimarkoFlow release packages", NIMARKOFLOW_VERSION, latest_version);
    ...
    if (!download_with_retry(release.backend_url, backend_file, release.backend_name) ||
        ...
        action_fail("nimarkoflow", "install", "Failed to download NimarkoFlow release packages", NIMARKOFLOW_VERSION, latest_version);

    if (!run_logged("Installing LuCI app package " + release.app_name, pkg_install_files_command([ app_file ])))
        action_fail("nimarkoflow", "install", "Failed to install LuCI app package", NIMARKOFLOW_VERSION, latest_version);
    if (i18n_file != "" && !run_logged("Installing LuCI Russian i18n package " + release.i18n_name, pkg_install_files_command([ i18n_file ])))
        action_fail("nimarkoflow", "install", "Failed to install LuCI Russian i18n package", NIMARKOFLOW_VERSION, latest_version);
    if (!run_logged("Installing NimarkoFlow package " + release.backend_name, pkg_install_files_command([ backend_file ])))
        action_fail("nimarkoflow", "install", "Failed to install NimarkoFlow package", NIMARKOFLOW_VERSION, latest_version);
    ...
    restart_nimarkoflow_after_successful_change();
    clear_version_caches();
    let new_version = installed_package_version("nimarkoflow");
    if (new_version == "")
        new_version = latest_version;
    updates_log("NimarkoFlow updated to " + new_version);
    action_success("nimarkoflow", "install", "NimarkoFlow has been installed", new_version, latest_version, 1, "latest", release.release_url);
}
...
function component_action(component, action) {
    ...
    capture_nimarkoflow_running_state();

    if (component == "nimarkoflow" && action == "check_update")
        check_nimarkoflow();
    else if (component == "nimarkoflow" && action == "install")
        install_nimarkoflow();
...
else if (mode == "latest-nimarkoflow-release-json")
    print(latest_nimarkoflow_release_json());
else if (mode == "latest-nimarkoflow-version")
    print(latest_nimarkoflow_version(), "\n");
else if (mode == "nimarkoflow-release-metadata")
    print(fetch_nimarkoflow_latest_release_metadata(), "\n");
else {
    warn("Usage: components/action.uc <component-action|latest-nimarkoflow-version|nimarkoflow-release-metadata> ...\n");
    exit(1);
}
