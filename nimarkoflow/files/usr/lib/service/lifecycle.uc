...
const CONFIG_NAME = getenv("NIMARKOFLOW_CONFIG_NAME") || constant_value("NIMARKOFLOW_CONFIG_NAME", "nimarkoflow");
const CONFIG_FILE = getenv("NIMARKOFLOW_CONFIG_FILE") || "/etc/config/" + CONFIG_NAME;
const LIB_DIR = getenv("NIMARKOFLOW_LIB") || "/usr/lib/nimarkoflow";
const BIN_PATH = getenv("NIMARKOFLOW_BIN") || constant_value("NIMARKOFLOW_BIN", "/usr/bin/nimarkoflow");
const SERVICE_INIT = getenv("NIMARKOFLOW_SERVICE_INIT") || constant_value("NIMARKOFLOW_SERVICE_INIT", "/etc/init.d/nimarkoflow");
const SERVICE_NAME = getenv("NIMARKOFLOW_SERVICE_NAME") || constant_value("NIMARKOFLOW_SERVICE_NAME", "nimarkoflow");
const LUCI_VIEW_DIR = getenv("NIMARKOFLOW_LUCI_VIEW_DIR") || constant_value("NIMARKOFLOW_LUCI_VIEW_DIR", "/www/luci-static/resources/view/nimarkoflow");
const LUCI_I18N_DOMAIN = getenv("NIMARKOFLOW_LUCI_I18N_DOMAIN") || constant_value("NIMARKOFLOW_LUCI_I18N_DOMAIN", "nimarkoflow");

const RUNTIME_STATE_DIR = getenv("NIMARKOFLOW_RUNTIME_STATE_DIR") || "/var/run/nimarkoflow";
const SYSTEM_INFO_CACHE_FILE = getenv("NIMARKOFLOW_SYSTEM_INFO_CACHE_FILE") || RUNTIME_STATE_DIR + "/system-info.json";
const RELOAD_STATE_FILE = getenv("NIMARKOFLOW_RELOAD_STATE_FILE") || RUNTIME_STATE_DIR + "/reload-state";
const RELOAD_STATE_SNAPSHOT_FILE = getenv("NIMARKOFLOW_RELOAD_STATE_SNAPSHOT_FILE") || RUNTIME_STATE_DIR + "/reload-state.snapshot." + clock()[0] + "." + clock()[1];
const PENDING_RELOAD_FILE = getenv("NIMARKOFLOW_PENDING_RELOAD_FILE") || RUNTIME_STATE_DIR + "/reload.pending";
const SERVICE_TRIGGER_SYNC_FILE = getenv("NIMARKOFLOW_SERVICE_TRIGGER_SYNC_FILE") || RUNTIME_STATE_DIR + "/service-triggers.sync";
const SUBSCRIPTION_UPDATE_STATE_DIR = getenv("NIMARKOFLOW_SUBSCRIPTION_UPDATE_STATE_DIR") || RUNTIME_STATE_DIR + "/subscription-update";
const SUBSCRIPTION_LINKS_DIR = getenv("NIMARKOFLOW_SUBSCRIPTION_LINKS_DIR") || RUNTIME_STATE_DIR + "/subscription-links";
const SUBSCRIPTION_METADATA_DIR = getenv("NIMARKOFLOW_SUBSCRIPTION_METADATA_DIR") || RUNTIME_STATE_DIR + "/subscription-metadata";
const OUTBOUND_METADATA_DIR = getenv("NIMARKOFLOW_OUTBOUND_METADATA_DIR") || RUNTIME_STATE_DIR + "/outbound-metadata";
const SECTION_CACHE_DIR = getenv("NIMARKOFLOW_SECTION_CACHE_DIR") || RUNTIME_STATE_DIR + "/section-cache";
const RULE_CONDITION_CACHE_DIR = getenv("NIMARKOFLOW_RULE_CONDITION_CACHE_DIR") || RUNTIME_STATE_DIR + "/rule-condition-cache";
const RUNTIME_CACHE_FORMAT_FILE = getenv("NIMARKOFLOW_RUNTIME_CACHE_FORMAT_FILE") || RUNTIME_STATE_DIR + "/cache-format";
const PERSISTENT_SUBSCRIPTION_CACHE_DIR = getenv("NIMARKOFLOW_PERSISTENT_SUBSCRIPTION_CACHE_DIR") || "/etc/nimarkoflow/subscription-cache";
const PERSISTENT_SUBSCRIPTION_CACHE_FORMAT_FILE = getenv("NIMARKOFLOW_PERSISTENT_SUBSCRIPTION_CACHE_FORMAT_FILE") || PERSISTENT_SUBSCRIPTION_CACHE_DIR + "/cache-format";
const PERSISTENT_SUBSCRIPTION_CACHE_FORMAT = getenv("NIMARKOFLOW_PERSISTENT_SUBSCRIPTION_CACHE_FORMAT") || "7";
const SUBSCRIPTION_BOOTSTRAP_RETRY_PID_FILE = getenv("NIMARKOFLOW_SUBSCRIPTION_BOOTSTRAP_RETRY_PID_FILE") || RUNTIME_STATE_DIR + "/subscription-bootstrap-retry.pid";
const DNS_FAILOVER_STATE_FILE = getenv("NIMARKOFLOW_DNS_FAILOVER_STATE_FILE") || RUNTIME_STATE_DIR + "/dns-failover.json";
const DNS_FAILOVER_PID_FILE = getenv("NIMARKOFLOW_DNS_FAILOVER_PID_FILE") || RUNTIME_STATE_DIR + "/dns-failover.pid";
const SUBSCRIPTION_UPDATE_LOCK_DIR = getenv("NIMARKOFLOW_SUBSCRIPTION_UPDATE_LOCK_DIR") || RUNTIME_STATE_DIR + "/subscription-update.lock";
const RELOAD_LOCK_DIR = getenv("NIMARKOFLOW_RELOAD_LOCK_DIR") || "/var/run/nimarkoflow.reload.lock";
const INTERNAL_CONFIG_TRIGGER_GUARD = getenv("NIMARKOFLOW_INTERNAL_CONFIG_TRIGGER_GUARD") || "/var/run/nimarkoflow.internal-config-change";

const LIST_UPDATE_CRON_MARKER = getenv("NIMARKOFLOW_LIST_UPDATE_CRON_MARKER") || "# nimarkoflow-list-update";
const SUBSCRIPTION_UPDATE_CRON_MARKER = getenv("NIMARKOFLOW_SUBSCRIPTION_UPDATE_CRON_MARKER") || "# nimarkoflow-subscription-update";
const COMPONENT_UPDATE_CHECK_CRON_MARKER = getenv("NIMARKOFLOW_COMPONENT_UPDATE_CHECK_CRON_MARKER") || "# nimarkoflow-component-update-check";
const RELOAD_STATE_FORMAT = int(getenv("NIMARKOFLOW_RELOAD_STATE_FORMAT") || "1");
const RUNTIME_CACHE_FORMAT = int(getenv("NIMARKOFLOW_RUNTIME_CACHE_FORMAT") || "8");
const RUNTIME_STABLE_MIN_AGE = int(getenv("NIMARKOFLOW_RUNTIME_STABLE_MIN_AGE") || "2");
const SING_BOX_START_STABLE_MIN_AGE = int(getenv("NIMARKOFLOW_SING_BOX_START_STABLE_MIN_AGE") || "8");
const SING_BOX_START_VERIFY_TIMEOUT = int(getenv("NIMARKOFLOW_SING_BOX_START_VERIFY_TIMEOUT") || "10");
const NFT_POPULATE_ENABLED_DEFAULT = int(getenv("NIMARKOFLOW_NFT_POPULATE_ENABLED") || "1");
...
const RT_TABLE_NAME = constant_value("RT_TABLE_NAME", "nimarkoflow");
const NFT_TABLE_NAME = constant_value("NFT_TABLE_NAME", "NimarkoFlowTable");
...
const NFT_COMMON_SET_NAME = constant_value("NFT_COMMON_SET_NAME", "nimarkoflow_subnets");
const NFT_COMMON6_SET_NAME = constant_value("NFT_COMMON6_SET_NAME", "nimarkoflow_subnets6");
const NFT_PORT_SET_NAME = constant_value("NFT_PORT_SET_NAME", "nimarkoflow_ports");
const NFT_IP_PORT_SET_NAME = constant_value("NFT_IP_PORT_SET_NAME", "nimarkoflow_ip_ports");
const NFT_IP_PORT6_SET_NAME = constant_value("NFT_IP_PORT6_SET_NAME", "nimarkoflow_ip6_ports");
const NFT_INTERFACE_SET_NAME = constant_value("NFT_INTERFACE_SET_NAME", "nimarkoflow_interfaces");
...
const SB_VARIANT_STATE_FILE = constant_value("SB_VARIANT_STATE_FILE", "/etc/nimarkoflow/sing-box-variant");
const SB_VERSION_STATE_FILE = constant_value("SB_VERSION_STATE_FILE", "/etc/nimarkoflow/sing-box-version");
...
let subscription_caches_prepared = getenv("NIMARKOFLOW_SUBSCRIPTION_CACHES_PREPARED") || "0";
let subscription_runtime_no_refresh = getenv("NIMARKOFLOW_SUBSCRIPTION_RUNTIME_NO_REFRESH") || "0";
...
function lifecycle_env() {
    return {
        NIMARKOFLOW_CONFIG_NAME: CONFIG_NAME,
        NIMARKOFLOW_LIB: LIB_DIR,
        NIMARKOFLOW_BIN: BIN_PATH,
        NIMARKOFLOW_SERVICE_INIT: SERVICE_INIT,
        NIMARKOFLOW_RUNTIME_STATE_DIR: RUNTIME_STATE_DIR,
        NIMARKOFLOW_SYSTEM_INFO_CACHE_FILE: SYSTEM_INFO_CACHE_FILE,
        NIMARKOFLOW_SUBSCRIPTION_UPDATE_STATE_DIR: SUBSCRIPTION_UPDATE_STATE_DIR,
        NIMARKOFLOW_SUBSCRIPTION_LINKS_DIR: SUBSCRIPTION_LINKS_DIR,
        NIMARKOFLOW_SUBSCRIPTION_METADATA_DIR: SUBSCRIPTION_METADATA_DIR,
        NIMARKOFLOW_OUTBOUND_METADATA_DIR: OUTBOUND_METADATA_DIR,
        NIMARKOFLOW_SECTION_CACHE_DIR: SECTION_CACHE_DIR,
        NIMARKOFLOW_RULE_CONDITION_CACHE_DIR: RULE_CONDITION_CACHE_DIR,
        NIMARKOFLOW_RUNTIME_CACHE_FORMAT_FILE: RUNTIME_CACHE_FORMAT_FILE,
        NIMARKOFLOW_RUNTIME_CACHE_FORMAT: as_string(RUNTIME_CACHE_FORMAT),
        NIMARKOFLOW_PERSISTENT_SUBSCRIPTION_CACHE_DIR: PERSISTENT_SUBSCRIPTION_CACHE_DIR,
        NIMARKOFLOW_PERSISTENT_SUBSCRIPTION_CACHE_FORMAT_FILE: PERSISTENT_SUBSCRIPTION_CACHE_FORMAT_FILE,
        NIMARKOFLOW_PERSISTENT_SUBSCRIPTION_CACHE_FORMAT: PERSISTENT_SUBSCRIPTION_CACHE_FORMAT,
        NIMARKOFLOW_SUBSCRIPTION_BOOTSTRAP_RETRY_PID_FILE: SUBSCRIPTION_BOOTSTRAP_RETRY_PID_FILE,
        NIMARKOFLOW_DNS_FAILOVER_STATE_FILE: DNS_FAILOVER_STATE_FILE,
        NIMARKOFLOW_DNS_FAILOVER_PID_FILE: DNS_FAILOVER_PID_FILE,
        NIMARKOFLOW_SUBSCRIPTION_UPDATE_LOCK_DIR: SUBSCRIPTION_UPDATE_LOCK_DIR,
        NIMARKOFLOW_PENDING_RELOAD_FILE: PENDING_RELOAD_FILE,
        NIMARKOFLOW_RELOAD_LOCK_DIR: RELOAD_LOCK_DIR,
        ...
        NIMARKOFLOW_RULE_CONDITION_CACHE_ENABLED: as_string(rule_condition_cache_enabled)
    };
}
...
function dnsmasq_has_nimarkoflow_managed_state() {
...
function start_main() {
    ...
    log_message("Starting NimarkoFlow", "info");
...
function start_impl() {
    ...
    else if (dnsmasq_has_nimarkoflow_managed_state()) {
...
function stop_main() {
    ...
    log_message("Stopping NimarkoFlow", "info");
...
function cleanup_failed_runtime() {
    ...
    log_message("Cleaning up NimarkoFlow runtime after failed start/reload", "info");
    ...
    if (status != 0)
        log_message("Failed to fully clean up NimarkoFlow runtime after start/reload failure", "warn");
...
function start() {
    ...
    if (status != 0) {
        log_message("Startup verification failed after NimarkoFlow was started; rolling back DNS changes", "warn");
        cleanup_failed_runtime();
        return status;
    }
...
function restart_runtime_for_reload() {
    ...
    log_message("Reload requires a full NimarkoFlow runtime restart", "info");
    ...
    if (status != 0) {
        log_message("Reload runtime restart verification failed after NimarkoFlow was started; rolling back DNS changes", "fatal");
        cleanup_failed_runtime();
        return status;
    }
...
function reload(reason) {
    ...
    log_message("Reloading NimarkoFlow", "info");
    ...
    if (!module_success(STATE_UC, [ "nimarkoflow-running", RT_TABLE_NAME, NFT_TABLE_NAME, NFT_FAKEIP_MARK ])) {
        log_message("Runtime state is incomplete; restarting NimarkoFlow runtime", "info");
    ...
    let dnsmasq_managed_state = dnsmasq_has_nimarkoflow_managed_state() ? 1 : 0;
    ...
    if (as_string(getenv("NIMARKOFLOW_RUNTIME_CACHE_INVALIDATED") || "0") == "1" ||
    ...
    if (plan_result.status != 0) {
        if (plan_result.status == 2) {
            log_message("Reload state is unavailable; restarting NimarkoFlow runtime", "info");
            return finish_reload_status(restart_runtime_for_reload(), reload_config_fingerprint);
        }
    ...
    if (plan.needs_sing_box_reload == 1) {
        ...
        if (status != 0) {
            log_message("Reload verification failed after sing-box was reloaded; stopping NimarkoFlow runtime", "fatal");
            cleanup_failed_runtime();
            return status;
        }
        status = module_status(PRIORITY_UC, [ "start-runtime" ]);
...
function reload_tracked(reason) {
    if (as_string(getenv("NIMARKOFLOW_UI_ACTION_TRACKED") || "0") == "1")
...
function restart() {
    log_message("Restarting NimarkoFlow", "info");
    ...
    log_message("Restart verification failed after NimarkoFlow was started; stopping NimarkoFlow runtime", "fatal");
    cleanup_failed_runtime();
    return 1;
}
...
function uninstall() {
    log_message("Uninstalling NimarkoFlow", "info");
    ...
    if (fs.stat("/etc/init.d/nimarkoflow") != null) {
        command_success_from_args([ "/etc/init.d/nimarkoflow", "stop" ]);
        command_success_from_args([ "/etc/init.d/nimarkoflow", "disable" ]);
    }

    package_manager_remove_if_installed("luci-i18n-nimarkoflow-ru");
    package_manager_remove_if_installed("luci-app-nimarkoflow");
    ...
    command_success_from_args([ "rm", "-rf", "/usr/lib/nimarkoflow" ]);
    command_success_from_args([ "rm", "-rf", LUCI_VIEW_DIR ]);
    remove_file(SERVICE_INIT);
    remove_file(BIN_PATH);
    remove_file("/usr/share/luci/menu.d/luci-app-nimarkoflow.json");
    remove_file("/usr/share/rpcd/acl.d/luci-app-nimarkoflow.json");
    remove_file("/etc/uci-defaults/50_luci-nimarkoflow");
