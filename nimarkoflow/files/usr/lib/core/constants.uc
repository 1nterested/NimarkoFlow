...
function constants_map() {
    ...
    c.NIMARKOFLOW_VERSION = env("NIMARKOFLOW_VERSION", "__COMPILED_VERSION_VARIABLE__");
    c.NIMARKOFLOW_CONFIG_NAME = env("NIMARKOFLOW_CONFIG_NAME", "nimarkoflow");
    c.NIMARKOFLOW_CONFIG = env("NIMARKOFLOW_CONFIG", "/etc/config/" + c.NIMARKOFLOW_CONFIG_NAME);
    c.NIMARKOFLOW_BIN = env("NIMARKOFLOW_BIN", "/usr/bin/nimarkoflow");
    c.NIMARKOFLOW_SERVICE_NAME = env("NIMARKOFLOW_SERVICE_NAME", "nimarkoflow");
    c.NIMARKOFLOW_SERVICE_INIT = env("NIMARKOFLOW_SERVICE_INIT", "/etc/init.d/nimarkoflow");
    c.NIMARKOFLOW_RELEASE_REPO = env("NIMARKOFLOW_RELEASE_REPO", "");
    c.NIMARKOFLOW_LUCI_VIEW_NAMESPACE = env("NIMARKOFLOW_LUCI_VIEW_NAMESPACE", "nimarkoflow");
    c.NIMARKOFLOW_LUCI_VIEW_DIR = env("NIMARKOFLOW_LUCI_VIEW_DIR", "/www/luci-static/resources/view/" + c.NIMARKOFLOW_LUCI_VIEW_NAMESPACE);
    c.NIMARKOFLOW_LUCI_I18N_DOMAIN = env("NIMARKOFLOW_LUCI_I18N_DOMAIN", "nimarkoflow");
    ...
    c.RT_TABLE_NAME = env("RT_TABLE_NAME", "nimarkoflow");

    c.NFT_TABLE_NAME = env("NFT_TABLE_NAME", "NimarkoFlowTable");
    c.NFT_LOCALV4_SET_NAME = env("NFT_LOCALV4_SET_NAME", "localv4");
    c.NFT_LOCALV6_SET_NAME = env("NFT_LOCALV6_SET_NAME", "localv6");
    c.NFT_COMMON_SET_NAME = env("NFT_COMMON_SET_NAME", "nimarkoflow_subnets");
    c.NFT_COMMON6_SET_NAME = env("NFT_COMMON6_SET_NAME", "nimarkoflow_subnets6");
    c.NFT_PORT_SET_NAME = env("NFT_PORT_SET_NAME", "nimarkoflow_ports");
    c.NFT_IP_PORT_SET_NAME = env("NFT_IP_PORT_SET_NAME", "nimarkoflow_ip_ports");
    c.NFT_IP_PORT6_SET_NAME = env("NFT_IP_PORT6_SET_NAME", "nimarkoflow_ip6_ports");
    c.NFT_DISCORD_SET_NAME = env("NFT_DISCORD_SET_NAME", "nimarkoflow_discord_subnets");
    c.NFT_DISCORD6_SET_NAME = env("NFT_DISCORD6_SET_NAME", "nimarkoflow_discord_subnets6");
    c.NFT_INTERFACE_SET_NAME = env("NFT_INTERFACE_SET_NAME", "nimarkoflow_interfaces");
    ...
    c.SB_MANAGED_SERVICE_MARKER = env("SB_MANAGED_SERVICE_MARKER", "NimarkoFlow managed sing-box service for binary variants");
    ...
    c.SB_VARIANT_STATE_FILE = env("SB_VARIANT_STATE_FILE", "/etc/nimarkoflow/sing-box-variant");
    c.SB_VERSION_STATE_FILE = env("SB_VERSION_STATE_FILE", "/etc/nimarkoflow/sing-box-version");
    ...
    c.ZAPRET_LEGACY_RUNTIME_BASE_DIR = env("ZAPRET_LEGACY_RUNTIME_BASE_DIR", "/var/run/nimarkoflow/zapret-runtime");
    c.ZAPRET_NFQWS_BIN = env("ZAPRET_NFQWS_BIN", c.ZAPRET_PROVIDER_NFQWS_BIN);
    c.ZAPRET_STATE_DIR = env("ZAPRET_STATE_DIR", "/var/run/nimarkoflow/zapret");
    ...
    c.ZAPRET2_STATE_DIR = env("ZAPRET2_STATE_DIR", "/var/run/nimarkoflow/zapret2");
    ...
    c.BYEDPI_STATE_DIR = env("BYEDPI_STATE_DIR", "/var/run/nimarkoflow/byedpi");
