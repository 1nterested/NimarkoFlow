...
const LIST_UPDATE_STATE_FILE = getenv("NIMARKOFLOW_LIST_UPDATE_STATE_FILE") || RUNTIME_STATE_DIR + "/list-update.timestamp";
const LIST_UPDATE_PID_FILE = getenv("NIMARKOFLOW_LIST_UPDATE_PID_FILE") || "/var/run/nimarkoflow_list_update.pid";
...
const SUBSCRIPTION_JOB_DIR = getenv("NIMARKOFLOW_SUBSCRIPTION_UPDATE_JOB_DIR") || "/var/run/nimarkoflow/subscription-update-jobs";
...
const RUNTIME_CACHE_FORMAT = getenv("NIMARKOFLOW_RUNTIME_CACHE_FORMAT") || "8";
...
const RELOAD_STATE_FORMAT = getenv("NIMARKOFLOW_RELOAD_STATE_FORMAT") || "1";
...
const PRIORITY_UC = getenv("NIMARKOFLOW_PRIORITY_UC") || LIB_DIR + "/singbox/priority.uc";
const DNS_FAILOVER_UC = getenv("NIMARKOFLOW_DNS_FAILOVER_UC") || LIB_DIR + "/singbox/dns_failover.uc";
const COMPONENT_JOB_DIR = getenv("UPDATES_JOB_DIR") || getenv("NIMARKOFLOW_UI_COMPONENT_ACTION_DIR") || "/var/run/nimarkoflow/component-actions";
const COMPONENT_UPDATE_CHECK_CACHE_DIR = getenv("NIMARKOFLOW_COMPONENT_UPDATE_CHECK_CACHE_DIR") || RUNTIME_STATE_DIR + "/component-update-checks";
const COMPONENT_UPDATE_CHECK_STATE_FILE = getenv("NIMARKOFLOW_COMPONENT_UPDATE_CHECK_STATE_FILE") || RUNTIME_STATE_DIR + "/component-update-check.timestamp";
const COMPONENT_UPDATE_CHECK_LOCK_DIR = getenv("NIMARKOFLOW_COMPONENT_UPDATE_CHECK_LOCK_DIR") || RUNTIME_STATE_DIR + "/component-update-check.lock";
...
const COMPONENT_JOB_STALE_GRACE_SECONDS = getenv("UPDATES_JOB_STALE_GRACE_SECONDS") || getenv("NIMARKOFLOW_UI_ACTION_STALE_GRACE_SECONDS") || "15";
const SUBSCRIPTION_JOB_FINISHED_TTL_MINUTES = getenv("NIMARKOFLOW_SUBSCRIPTION_UPDATE_JOB_FINISHED_TTL_MINUTES") || "60";
const SUBSCRIPTION_JOB_ORPHAN_OUTPUT_TTL_MINUTES = getenv("NIMARKOFLOW_SUBSCRIPTION_UPDATE_JOB_ORPHAN_OUTPUT_TTL_MINUTES") || "60";
const SUBSCRIPTION_JOB_STALE_GRACE_SECONDS = getenv("NIMARKOFLOW_UI_ACTION_STALE_GRACE_SECONDS") || "15";
const NFT_TABLE_NAME = getenv("NFT_TABLE_NAME") || "NimarkoFlowTable";
const NFT_COMMON_SET_NAME = getenv("NFT_COMMON_SET_NAME") || "nimarkoflow_subnets";
const NFT_COMMON6_SET_NAME = getenv("NFT_COMMON6_SET_NAME") || "nimarkoflow_subnets6";
const NFT_IP_PORT_SET_NAME = getenv("NFT_IP_PORT_SET_NAME") || "nimarkoflow_ip_ports";
const NFT_IP_PORT6_SET_NAME = getenv("NFT_IP_PORT6_SET_NAME") || "nimarkoflow_ip6_ports";
const NFT_DISCORD_SET_NAME = getenv("NFT_DISCORD_SET_NAME") || "nimarkoflow_discord_subnets";
const NFT_DISCORD6_SET_NAME = getenv("NFT_DISCORD6_SET_NAME") || "nimarkoflow_discord_subnets6";
const NFT_INTERFACE_SET_NAME = getenv("NFT_INTERFACE_SET_NAME") || "nimarkoflow_interfaces";
...
function ensure_subscription_runtime_dirs() {
    let command = command_env({
        ...
        NIMARKOFLOW_RUNTIME_CACHE_FORMAT: RUNTIME_CACHE_FORMAT,
...
function subscription_worker_env() {
    return {
        ...
        NIMARKOFLOW_RUNTIME_STATE_DIR: RUNTIME_STATE_DIR,
        NIMARKOFLOW_SUBSCRIPTION_UPDATE_STATE_DIR: SUBSCRIPTION_UPDATE_STATE_DIR,
        NIMARKOFLOW_SUBSCRIPTION_UPDATE_JOB_DIR: SUBSCRIPTION_JOB_DIR,
        NIMARKOFLOW_SUBSCRIPTION_LINKS_DIR: SUBSCRIPTION_LINKS_DIR,
        NIMARKOFLOW_SUBSCRIPTION_METADATA_DIR: SUBSCRIPTION_METADATA_DIR,
        NIMARKOFLOW_OUTBOUND_METADATA_DIR: OUTBOUND_METADATA_DIR,
        NIMARKOFLOW_SECTION_CACHE_DIR: SECTION_CACHE_DIR,
        ...
        NIMARKOFLOW_PERSISTENT_SUBSCRIPTION_CACHE_FORMAT_FILE: PERSISTENT_SUBSCRIPTION_CACHE_FORMAT_FILE,
        NIMARKOFLOW_PERSISTENT_SUBSCRIPTION_CACHE_FORMAT: PERSISTENT_SUBSCRIPTION_CACHE_FORMAT,
        NIMARKOFLOW_SUBSCRIPTION_UPDATE_JOB_FINISHED_TTL_MINUTES: SUBSCRIPTION_JOB_FINISHED_TTL_MINUTES,
        NIMARKOFLOW_SUBSCRIPTION_UPDATE_JOB_ORPHAN_OUTPUT_TTL_MINUTES: SUBSCRIPTION_JOB_ORPHAN_OUTPUT_TTL_MINUTES,
        NIMARKOFLOW_UI_ACTION_STALE_GRACE_SECONDS: SUBSCRIPTION_JOB_STALE_GRACE_SECONDS
    };
}
...
function valid_component_name(component) {
    ...
    return component == "nimarkoflow" || component == "sing_box" || component == "zapret" ||
...
function component_update_check_cache() {
    ...
    if (enabled) {
        for (let component in [ "nimarkoflow", "sing_box", "zapret", "zapret2", "byedpi" ]) {
...
function component_worker_env() {
    return {
        ...
        NIMARKOFLOW_UI_COMPONENT_ACTION_TRACKED: "1"
    };
}
...
function subscription_update_common(force, target_section, target_source_index) {
    ...
    if (!acquire_runtime_lock(RELOAD_LOCK_DIR, force)) {
        ...
        log_message("NimarkoFlow reload is already running; skipping subscription update", "info");
        if (force)
