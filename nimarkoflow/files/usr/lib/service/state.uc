...
const DEFAULT_PENDING_RELOAD_FILE = getenv("NIMARKOFLOW_PENDING_RELOAD_FILE") || "/var/run/nimarkoflow/reload.pending";
const DEFAULT_SERVICE_INIT = getenv("NIMARKOFLOW_SERVICE_INIT") || "/etc/init.d/nimarkoflow";
...
function run_pending_reload_if_requested(path, init_script) {
    ...
    command_success_from_args([ "logger", "-t", "nimarkoflow", "[info] Applying pending NimarkoFlow reload" ]);
...
function cleanup_rule_condition_cache(path) {
    ...
    system("rm -rf /tmp/nimarkoflow-rule-cache.* >/dev/null 2>&1");
}
...
function hup_sing_box_runtime() {
    ...
    command_success_from_args([ "logger", "-t", "nimarkoflow", "[info] Applying DNS failover with sing-box SIGHUP reload" ]);
...
function reload_sing_box_runtime(previous_pid, config_hash_before, config_hash_after, force) {
    if (!sing_box_runtime_reload_needed(config_hash_before, config_hash_after, force)) {
        command_success_from_args([ "logger", "-t", "nimarkoflow", "[info] sing-box reload skipped: configuration is unchanged" ]);
        return;
    }

    previous_pid = sing_box_reload_previous_pid(previous_pid, config_hash_before, config_hash_after);
    command_success_from_args([ "logger", "-t", "nimarkoflow", "[info] Reloading sing-box runtime" ]);
    if (!command_success_from_args([ "/etc/init.d/sing-box", "reload" ])) {
        command_success_from_args([ "logger", "-t", "nimarkoflow", "[fatal] Failed to reload sing-box. Aborted." ]);
        exit(1);
    }

    let timeout = getenv("NIMARKOFLOW_SING_BOX_RELOAD_PID_TIMEOUT") || "15";
    if (previous_pid > 0 && !wait_sing_box_pid_replacement(previous_pid, timeout)) {
        command_success_from_args([ "logger", "-t", "nimarkoflow", "[fatal] sing-box reload did not replace the running process. Aborted." ]);
        exit(1);
    }
}
...
function nimarkoflow_runtime_network_configured(rt_table, nft_table, mark) {
...
function nimarkoflow_running(rt_table, nft_table, mark) {
    return sing_box_service_running() && sing_box_runtime_ports_ready() &&
        nimarkoflow_runtime_network_configured(rt_table, nft_table, mark);
}

function nimarkoflow_stably_running(rt_table, nft_table, mark, min_age) {
    return sing_box_service_stable(min_age) && sing_box_runtime_ports_ready() &&
        nimarkoflow_runtime_network_configured(rt_table, nft_table, mark);
}

function wait_nimarkoflow_stable_start(rt_table, nft_table, mark, min_age, timeout) {
    timeout = int(timeout || 8);
    while (timeout > 0) {
        if (nimarkoflow_stably_running(rt_table, nft_table, mark, min_age))
            return true;
    ...
    return nimarkoflow_stably_running(rt_table, nft_table, mark, min_age);
}
...
function dnsmasq_signature_body(settings, dnsmasq, legacy_dnsmasq_present) {
    ...
    body = signature_add_value(body, "dhcp.@dnsmasq[0].nimarkoflow_server", option(dnsmasq, "nimarkoflow_server", ""));
    body = signature_add_value(body, "dhcp.@dnsmasq[0].nimarkoflow_noresolv", option(dnsmasq, "nimarkoflow_noresolv", ""));
    body = signature_add_value(body, "dhcp.@dnsmasq[0].nimarkoflow_cachesize", option(dnsmasq, "nimarkoflow_cachesize", ""));
    body = signature_add_value(body, "dhcp.nimarkoflow.present", arg_bool(legacy_dnsmasq_present) ? "1" : "0");
...
function uci_dnsmasq() {
    return {
        ...
        nimarkoflow_server: uci_get("dhcp.@dnsmasq[0].nimarkoflow_server"),
        nimarkoflow_noresolv: uci_get("dhcp.@dnsmasq[0].nimarkoflow_noresolv"),
        nimarkoflow_cachesize: uci_get("dhcp.@dnsmasq[0].nimarkoflow_cachesize")
    };
}
...
if (mode == "file-first-line")
...
else if (mode == "nimarkoflow-running")
    exit(nimarkoflow_running(ARGV[1], ARGV[2], ARGV[3]) ? 0 : 1);
else if (mode == "nimarkoflow-stably-running")
    exit(nimarkoflow_stably_running(ARGV[1], ARGV[2], ARGV[3], ARGV[4]) ? 0 : 1);
else if (mode == "wait-nimarkoflow-stable-start")
    exit(wait_nimarkoflow_stable_start(ARGV[1], ARGV[2], ARGV[3], ARGV[4], ARGV[5]) ? 0 : 1);
    ...
    exit(print_signature_hash(dnsmasq_signature_body(uci_settings(), uci_dnsmasq(), uci_exists("dhcp.nimarkoflow"))) ? 0 : 1);
