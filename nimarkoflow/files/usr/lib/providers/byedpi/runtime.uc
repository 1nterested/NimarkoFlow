...
const BYEDPI_STATE_DIR = getenv("BYEDPI_STATE_DIR") || "/var/run/nimarkoflow/byedpi";
...
function start_runtime() {
    ...
    if (standalone_service_enabled())
        log_message("Standalone byedpi service is enabled. NimarkoFlow manages ciadpi itself for action 'byedpi'; disable standalone byedpi autostart to avoid boot-time port conflicts.", "warn");

    if (standalone_service_running()) {
        log_message("Stopping standalone byedpi service before starting NimarkoFlow-managed ciadpi runtime", "info");
        command_success_from_args([ BYEDPI_SERVICE_INIT, "stop" ]);
        command_success_from_args([ "sleep", "1" ]);
        if (standalone_service_running()) {
            log_message("Standalone byedpi service is still running and may conflict with NimarkoFlow-managed ciadpi runtime. Aborted.", "fatal");
            exit(1);
        }
    }

    if (!ensure_runtime_dirs()) {
        log_message("Failed to prepare the NimarkoFlow ByeDPI state directory in " + BYEDPI_STATE_DIR + ". Aborted.", "fatal");
        exit(1);
    }
...
function status_json() {
    ...
    if (configured && !provider)
        ...
        message = "standalone byedpi service is active together with NimarkoFlow action=byedpi; port conflicts are possible";
    else if (configured && standalone_enabled)
        message = "standalone byedpi service autostart is enabled; disable it to avoid boot-time port conflicts with NimarkoFlow action=byedpi";
    else if (running > expected || supervisors > expected)
        message = "unexpected NimarkoFlow-managed ciadpi processes are running without matching action=byedpi rules";
    else if (configured && unstable)
        message = "NimarkoFlow-managed ciadpi has restarted after exiting; the ByeDPI strategy or traffic load may be unstable";
    else if (configured && !ready)
        message = "action=byedpi is configured, but the NimarkoFlow-managed ciadpi runtime is not ready";
