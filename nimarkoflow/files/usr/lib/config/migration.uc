...
const NIMARKOFLOW_INTERNAL_CONFIG_TRIGGER_GUARD = getenv("NIMARKOFLOW_INTERNAL_CONFIG_TRIGGER_GUARD") || "/var/run/nimarkoflow.internal-config-change";
...
function subscription_cache_paths(section) {
    return [
        ...
        NIMARKOFLOW_SUBSCRIPTION_METADATA_DIR + "/" + section + ".json",
        NIMARKOFLOW_SUBSCRIPTION_LINKS_DIR + "/" + section + ".json",
        NIMARKOFLOW_OUTBOUND_METADATA_DIR + "/" + section + ".json",
        NIMARKOFLOW_SECTION_CACHE_DIR + "/" + section + ".json",
...
// Podkop Plus -> NimarkoFlow source migration.
...
function migrate_nimarkoflow_model(model) {
    let ctx = migration_context(model);
    apply_migrations(ctx);
    return ctx;
}
...
function ensure_runtime_cache_format() {
    ...
    if (first_line(NIMARKOFLOW_RUNTIME_CACHE_FORMAT_FILE) != NIMARKOFLOW_RUNTIME_CACHE_FORMAT) {
        if (first_line(NIMARKOFLOW_PERSISTENT_SUBSCRIPTION_CACHE_FORMAT_FILE) == NIMARKOFLOW_PERSISTENT_SUBSCRIPTION_CACHE_FORMAT)
        ...
        ensure_runtime_dirs();
        fs.writefile(NIMARKOFLOW_RUNTIME_CACHE_FORMAT_FILE, NIMARKOFLOW_RUNTIME_CACHE_FORMAT + "\n");
    }

    if (first_line(NIMARKOFLOW_PERSISTENT_SUBSCRIPTION_CACHE_FORMAT_FILE) != NIMARKOFLOW_PERSISTENT_SUBSCRIPTION_CACHE_FORMAT) {
        run("rm -rf " + shell_quote(NIMARKOFLOW_PERSISTENT_SUBSCRIPTION_CACHE_DIR));
        ensure_dir(NIMARKOFLOW_PERSISTENT_SUBSCRIPTION_CACHE_DIR);
        run("chmod 700 " + shell_quote(NIMARKOFLOW_PERSISTENT_SUBSCRIPTION_CACHE_DIR) + " >/dev/null 2>&1");
        fs.writefile(NIMARKOFLOW_PERSISTENT_SUBSCRIPTION_CACHE_FORMAT_FILE, NIMARKOFLOW_PERSISTENT_SUBSCRIPTION_CACHE_FORMAT + "\n");
        run("chmod 600 " + shell_quote(NIMARKOFLOW_PERSISTENT_SUBSCRIPTION_CACHE_FORMAT_FILE) + " >/dev/null 2>&1");
    }
}

function remove_legacy_server_country_cache() {
    fs.unlink(NIMARKOFLOW_RUNTIME_STATE_DIR + "/server-country-cache.json");
}
...
function mark_internal_config_guard() {
    ...
    if (hash == "") {
        fs.unlink(NIMARKOFLOW_INTERNAL_CONFIG_TRIGGER_GUARD);
        return;
    }

    let stamp = clock();
    let tmp_path = NIMARKOFLOW_INTERNAL_CONFIG_TRIGGER_GUARD + "." + stamp[0] + "." + stamp[1];
    fs.writefile(tmp_path, as_string(stamp[0]) + "\n" + hash + "\n");
    if (!fs.rename(tmp_path, NIMARKOFLOW_INTERNAL_CONFIG_TRIGGER_GUARD))
        fs.unlink(tmp_path);
}
...
function migrate_model(model, source) {
    return source == "podkop"
        ...
        : migrate_nimarkoflow_model(model);
}
...
function main(argv) {
    ...
    warn("       config/migration.uc migrate-fixture <fixture.json> [nimarkoflow|podkop]\n");
    return 1;
}
...
function module_exports() {
    return {
        ...
        migrate_nimarkoflow_model: migrate_nimarkoflow_model,
