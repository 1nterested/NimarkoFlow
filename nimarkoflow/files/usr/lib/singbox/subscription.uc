...
const NIMARKOFLOW_RUNTIME_CACHE_FORMAT = int(getenv("NIMARKOFLOW_RUNTIME_CACHE_FORMAT") || "8");
...
let section_cache_dir = NIMARKOFLOW_SECTION_CACHE_DIR;
...
function legacy_metadata_path(section_name) {
    return NIMARKOFLOW_SUBSCRIPTION_METADATA_DIR + "/" + section_name + ".json";
}

function persistent_source_path(source_section, suffix) {
    return NIMARKOFLOW_PERSISTENT_SUBSCRIPTION_CACHE_DIR + "/" + source_section + suffix;
}
...
function new_section_state(section_name) {
    return {
        version: NIMARKOFLOW_RUNTIME_CACHE_FORMAT,
