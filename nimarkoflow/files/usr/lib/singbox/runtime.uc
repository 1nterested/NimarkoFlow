...
const NFT_PORT_SET_NAME = getenv("NFT_PORT_SET_NAME") || "nimarkoflow_ports";
...
const SB_MANAGED_SERVICE_MARKER = getenv("SB_MANAGED_SERVICE_MARKER") || "NimarkoFlow managed sing-box service for binary variants";
...
function service_listen_address_value(settings) {
    ...
    log_message("Failed to determine the listening IP address. Please open an issue to report this problem: https://github.com//issues", "error");
    return "";
}
...
function save_config_file(temp_file_path, config_path) {
    if (!fs.chmod(temp_file_path, 0o600))
        return false;
    ...
    return fs.chmod(config_path, 0o600) == true;
}
