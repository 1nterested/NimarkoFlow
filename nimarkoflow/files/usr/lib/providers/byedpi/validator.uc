...
function controlled_option_failure(token, next_token) {
    ...
    if (token == "--ip" || index(token, "--ip=") == 0 || token == "-i" || match(token, /^-i.+/) != null ||
        ...
        return validation_failure(
            "ByeDPI listen address and port are assigned by NimarkoFlow and must not be set in the strategy: " + display,
            [ base ]
        );
    }
    ...
    if (token == "--transparent" || token == "-E" || match(token, /^-E.+/) != null)
        return validation_failure(
            "Transparent proxy mode is incompatible with action=byedpi because NimarkoFlow connects to ciadpi through SOCKS.",
            [ base ]
        );

    if (token == "--daemon" || token == "-D" || match(token, /^-D.+/) != null)
        return validation_failure(
            "NimarkoFlow manages the ciadpi process lifecycle itself, so daemon mode is not allowed here.",
            [ base ]
        );

    if (token == "--pidfile" || index(token, "--pidfile=") == 0 || token == "-w" || match(token, /^-w.+/) != null)
        return validation_failure(
            "NimarkoFlow manages ciadpi pid files itself, so pidfile options are not allowed here.",
            [ base ]
        );
