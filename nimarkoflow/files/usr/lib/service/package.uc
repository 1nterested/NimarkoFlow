...
const CONFIG_NAME = env("NIMARKOFLOW_CONFIG_NAME", "nimarkoflow");
const RT_TABLES_PATH = env("NIMARKOFLOW_RT_TABLES", "/etc/iproute2/rt_tables");
const BIN_PATH = env("NIMARKOFLOW_BIN", "/usr/bin/nimarkoflow");
const INIT_PATH = env("NIMARKOFLOW_INIT", "/etc/init.d/nimarkoflow");
const DNS_APPLY_UC = env("NIMARKOFLOW_DNS_APPLY_UC", "/usr/lib/nimarkoflow/dns/apply.uc");
const SING_BOX_INIT = env("NIMARKOFLOW_SING_BOX_INIT", "/etc/init.d/sing-box");
const SING_BOX_BIN = env("NIMARKOFLOW_SING_BOX_BIN", "/usr/bin/sing-box");
const SING_BOX_CRONET = env("NIMARKOFLOW_SING_BOX_CRONET", "/usr/lib/libcronet.so");
const SING_BOX_MANAGED_MARKER = env("SB_MANAGED_SERVICE_MARKER", "NimarkoFlow managed sing-box service for binary variants");
const PACKAGE_UPGRADE_STATE = env("NIMARKOFLOW_PACKAGE_UPGRADE_STATE", "/tmp/nimarkoflow-package-was-running");
const PACKAGE_TEST_MODE = env("NIMARKOFLOW_PACKAGE_TEST_MODE", "") != "";
...
function remove_rt_tables_entry() {
    ...
    for (let line in split(data, "\n")) {
        if (index(line, "105 nimarkoflow") >= 0) {
            changed = true;
            continue;
        }
        push(lines, line);
...
function luci_cache_globs() {
    let configured = env("NIMARKOFLOW_LUCI_CACHE_GLOBS", "");
    if (configured != "")
...
function luci_postinst() {
    ...
    if (!PACKAGE_TEST_MODE) {
        ...
        command_success_from_args([ "logger", "-t", "nimarkoflow", "[info] Package defaults applied" ]);
    }
    return true;
}
