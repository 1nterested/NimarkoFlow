...
const SERVICE_NAME = getenv("NIMARKOFLOW_SERVICE_NAME") || "nimarkoflow";
const STATE_UC = LIB_DIR + "/service/state.uc";
const UI_UC = LIB_DIR + "/service/ui.uc";
const STATE_DIR = getenv("NIMARKOFLOW_UI_STATE_DIR") || "/var/run/nimarkoflow/ui-state";
const PENDING_RELOAD_FILE = getenv("NIMARKOFLOW_PENDING_RELOAD_FILE") || "/var/run/nimarkoflow/reload.pending";
const SERVICE_ACTION_DIR = getenv("NIMARKOFLOW_UI_SERVICE_ACTION_DIR") || STATE_DIR + "/service-actions";
const SERVICE_ACTION_LOCK_DIR = getenv("NIMARKOFLOW_UI_SERVICE_ACTION_LOCK_DIR") || STATE_DIR + "/service-actions.lock";
const LATENCY_ACTION_DIR = getenv("NIMARKOFLOW_UI_LATENCY_ACTION_DIR") || STATE_DIR + "/latency-actions";
const COMPONENT_ACTION_DIR = getenv("NIMARKOFLOW_UI_COMPONENT_ACTION_DIR") || getenv("UPDATES_JOB_DIR") || "/var/run/nimarkoflow/component-actions";
const SUBSCRIPTION_ACTION_DIR = getenv("NIMARKOFLOW_UI_SUBSCRIPTION_ACTION_DIR") || getenv("NIMARKOFLOW_SUBSCRIPTION_UPDATE_JOB_DIR") || "/var/run/nimarkoflow/subscription-update-jobs";
const SING_BOX_VERSION_CACHE_FILE = getenv("NIMARKOFLOW_UI_SING_BOX_VERSION_CACHE_FILE") || STATE_DIR + "/sing-box-version";
const SING_BOX_VERSION_CACHE_LOCK_DIR = getenv("NIMARKOFLOW_UI_SING_BOX_VERSION_CACHE_LOCK_DIR") || SING_BOX_VERSION_CACHE_FILE + ".lock";
const SING_BOX_VARIANT_STATE_FILE = getenv("NIMARKOFLOW_UI_SING_BOX_VARIANT_STATE_FILE") || "/etc/nimarkoflow/sing-box-variant";
const SING_BOX_BIN_PATH = getenv("NIMARKOFLOW_UI_SING_BOX_BIN_PATH") || "/usr/bin/sing-box";
const SING_BOX_VERSION_PROBE_TIMEOUT_SECONDS = getenv("NIMARKOFLOW_UI_SING_BOX_VERSION_PROBE_TIMEOUT_SECONDS") || "1";
const SING_BOX_VERSION_PROBE_FAILURE_TTL_SECONDS = getenv("NIMARKOFLOW_UI_SING_BOX_VERSION_PROBE_FAILURE_TTL_SECONDS") || "30";
const ACTION_FINISHED_TTL_MINUTES = getenv("NIMARKOFLOW_UI_ACTION_FINISHED_TTL_MINUTES") || "60";
const ACTION_ACKED_TTL_SECONDS = getenv("NIMARKOFLOW_UI_ACTION_ACKED_TTL_SECONDS") || "15";
const ACTION_STALE_GRACE_SECONDS = getenv("NIMARKOFLOW_UI_ACTION_STALE_GRACE_SECONDS") || "15";
const SERVICE_ACTION_TIMEOUT_SECONDS = getenv("NIMARKOFLOW_UI_SERVICE_ACTION_TIMEOUT_SECONDS") || "120";
const SERVICE_ACTION_SETTLE_SECONDS = getenv("NIMARKOFLOW_UI_SERVICE_ACTION_SETTLE_SECONDS") || "2";
const RUNTIME_STABLE_MIN_AGE = getenv("NIMARKOFLOW_RUNTIME_STABLE_MIN_AGE") || "2";
const NFT_TABLE_NAME = getenv("NFT_TABLE_NAME") || "NimarkoFlowTable";
const RT_TABLE_NAME = getenv("RT_TABLE_NAME") || "nimarkoflow";
...
function ui_state_json() {
    ...
    let nimarkoflow_running = arg_number(ARGV[1]);
    let nimarkoflow_enabled = arg_number(ARGV[2]);
    let nimarkoflow_status = as_string(ARGV[3]);
    let nimarkoflow_dns_configured = arg_number(ARGV[4]);
    let sing_box_running = arg_number(ARGV[5]);
    let sing_box_enabled = arg_number(ARGV[6]);
    let sing_box_status = as_string(ARGV[7]);

    if (nimarkoflow_status == "")
        nimarkoflow_status = service_status_text(nimarkoflow_running, nimarkoflow_enabled);
    ...
    write_json({
        service: {
            nimarkoflow: {
                running: nimarkoflow_running,
                enabled: nimarkoflow_enabled,
                status: nimarkoflow_status,
                dns_configured: nimarkoflow_dns_configured
            },
            sing_box: {
...
function current_ui_state_json() {
    ...
    let nimarkoflow_is_running = nimarkoflow_running() ? 1 : 0;
    let nimarkoflow_is_enabled = service_enabled() ? 1 : 0;
    let sing_box_is_running = nimarkoflow_is_running ? 1 : (sing_box_running() ? 1 : 0);
    let sing_box_is_enabled = sing_box_enabled() ? 1 : 0;
    let nimarkoflow_status = service_status_text(nimarkoflow_is_running, nimarkoflow_is_enabled);
    ...
    if (active_action == "start")
        nimarkoflow_status = "starting";
    else if (active_action == "stop")
        nimarkoflow_status = "stopping";
    else if (active_action == "restart")
        nimarkoflow_status = "restarting";
    else if (active_action == "reload")
        nimarkoflow_status = "reloading";

    write_json({
        service: {
            nimarkoflow: {
                running: nimarkoflow_is_running,
                enabled: nimarkoflow_is_enabled,
                status: nimarkoflow_status,
...
function service_action_reached_expected_state(action) {
    ...
    if (expected < 0)
        return false;
    return expected == 1 ? nimarkoflow_running() : !nimarkoflow_running();
}
...
function launch_worker(args) {
    ...
    let command = command_env({
        ...
        NIMARKOFLOW_SERVICE_NAME: SERVICE_NAME,
        NIMARKOFLOW_UI_STATE_DIR: STATE_DIR,
        NIMARKOFLOW_UI_SERVICE_ACTION_DIR: SERVICE_ACTION_DIR,
        NIMARKOFLOW_UI_SERVICE_ACTION_LOCK_DIR: SERVICE_ACTION_LOCK_DIR,
        NIMARKOFLOW_UI_LATENCY_ACTION_DIR: LATENCY_ACTION_DIR,
        NIMARKOFLOW_UI_COMPONENT_ACTION_DIR: COMPONENT_ACTION_DIR,
        NIMARKOFLOW_UI_SUBSCRIPTION_ACTION_DIR: SUBSCRIPTION_ACTION_DIR,
        NIMARKOFLOW_UI_SING_BOX_VERSION_CACHE_FILE: SING_BOX_VERSION_CACHE_FILE,
        NIMARKOFLOW_UI_SING_BOX_VERSION_CACHE_LOCK_DIR: SING_BOX_VERSION_CACHE_LOCK_DIR,
        NIMARKOFLOW_UI_SING_BOX_VARIANT_STATE_FILE: SING_BOX_VARIANT_STATE_FILE,
        NIMARKOFLOW_UI_SING_BOX_BIN_PATH: SING_BOX_BIN_PATH,
        NIMARKOFLOW_UI_SING_BOX_VERSION_PROBE_TIMEOUT_SECONDS: SING_BOX_VERSION_PROBE_TIMEOUT_SECONDS,
        NIMARKOFLOW_UI_SING_BOX_VERSION_PROBE_FAILURE_TTL_SECONDS: SING_BOX_VERSION_PROBE_FAILURE_TTL_SECONDS,
        NIMARKOFLOW_UI_ACTION_FINISHED_TTL_MINUTES: ACTION_FINISHED_TTL_MINUTES,
        NIMARKOFLOW_UI_ACTION_ACKED_TTL_SECONDS: ACTION_ACKED_TTL_SECONDS,
        NIMARKOFLOW_UI_ACTION_STALE_GRACE_SECONDS: ACTION_STALE_GRACE_SECONDS,
        NIMARKOFLOW_UI_SERVICE_ACTION_TIMEOUT_SECONDS: SERVICE_ACTION_TIMEOUT_SECONDS,
        NIMARKOFLOW_UI_SERVICE_ACTION_SETTLE_SECONDS: SERVICE_ACTION_SETTLE_SECONDS,
...
function run_pending_reload_after_service_action(action, success) {
    ...
    command_success_from_args([ "logger", "-t", SERVICE_NAME, "[info] Applying pending NimarkoFlow reload" ]);
...
function finish_service_action_after_command(action, job_id_value, status, spawn_waiter) {
    ...
    if (!service_enabled() && !nimarkoflow_running()) {
...
function service_action_worker(path, action, job_id_value, reason) {
    ...
    if (reason != "")
        push(args, reason);
    let command = "NIMARKOFLOW_UI_ACTION_TRACKED=1 " + command_from_args(args) + " >/dev/null 2>&1";
